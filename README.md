...
Context budgeting, queue backpressure, Ollama recovery, and trading intent
deduplication are documented in
[docs/INFERENCE_TRADING_RELIABILITY.md](./docs/INFERENCE_TRADING_RELIABILITY.md).
...
- `orchestration.interactive_pool_max_in_flight` and `orchestration.solver_pool_max_in_flight` split global concurrency by workload class, so UI/direct/trading traffic cannot starve solver workflows (and vice versa). `orchestration.workload_pool_wait_timeout_ms` bounds global waits, while `candidate_queue_wait_timeout_ms` provides event-driven backpressure for provider/shared-host capacity.
- Oversized local-model histories are compacted before dispatch using the configured provider context window (or `default_local_context_window_tokens`). Gail preserves system policy and recent context, reserves output/safety tokens, and records an explicit omitted-history summary.
- Workflow model floors are configurable with `orchestration.interactive_model_floor_b` and `orchestration.solver_model_floor_b`. When `orchestration.strict_no_downgrade` is enabled, Gail rejects responses that fall below the configured/requested model size floor.
- Trading advisories use a reserved `orchestration.trading_pool_max_in_flight` capacity (or `GAIL_TRADING_POOL_MAX_IN_FLIGHT`) and are classified ahead of solver/research work. This keeps long Refiner/Conductor requests from consuming the advisory queue while candidate-level limits still enforce one request per native node.
- Admission waits are workload-specific: `interactive_pool_wait_timeout_ms` is kept short, while `solver_pool_wait_timeout_ms` and `trading_pool_wait_timeout_ms` provide long-lived independent queues. The class-specific `GAIL_*_POOL_WAIT_TIMEOUT_MS` variables override these values without changing per-candidate/native-host concurrency.
- Trading live execution can be safety-gated with `trading.live_execution_auto_gate_enabled`: the current build must have a valid paper-qualification window and a profitable latest backtest before Gail promotes paper intents to orders. A missing, expired, or failed gate leaves execution in paper/hold mode.
...
- When a request includes an explicit Ollama `base_url` (such as a provider profile bound to one integration pod), Gail keeps fallback scoped to that endpoint by default and does not fan out across other Ollama integration endpoints unless `GAIL_OLLAMA_ALLOW_CROSS_ENDPOINT_FALLBACK=true` is set.
- Local Ollama generation is tuned for higher-latency local inference by default: endpoint-scoped `GAIL_OLLAMA_MAX_CONCURRENT_REQUESTS` defaults to `1`, `GAIL_OLLAMA_QUEUE_TIMEOUT_SECONDS` defaults to `20`, `GAIL_OLLAMA_SATURATION_BACKOFF_SECONDS` defaults to `20`, `GAIL_OLLAMA_TIMEOUT_SECONDS` defaults to `90`, `GAIL_OLLAMA_MAX_RETRIES` defaults to `0`, and `GAIL_OLLAMA_MAX_PREDICT` defaults to `512`. Queue-saturation signals place the affected endpoint into cooldown (and the whole Ollama family only when `GAIL_OLLAMA_FAMILY_SATURATION_BACKOFF=true`), while unrelated Ollama hosts continue in parallel. Runner exits receive one bounded respawn retry without extending the original request deadline.
- Ollama model resolution now excludes embedding/rerank models for `/api/generate` requests, so Gail will not route generation traffic to models such as `nomic-embed-text`.
- Ollama responses now emit local usage telemetry (`queue_wait_ms`, `inference_ms`, token estimates) inside response metadata. Gail feeds these metrics into candidate routing weights so queue-bound or slow local paths are down-ranked automatically.
...
- When every candidate is in transient cached backoff, Gail forces a bounded re-probe before degrading; tune probe width with `GAIL_TRANSIENT_BACKOFF_PROBE_CANDIDATES` (or `REFINER_AI_TRANSIENT_BACKOFF_PROBE_CANDIDATES`) to trade recovery speed vs. request fan-out.
