...
mod tests {
    ...
    #[tokio::test]
    async fn octobot_client_get_exchange_info_uses_trading_symbols_as_primary_market_status_source()
    {
        let server = MockServer::start().await;
        ...
        assert_eq!(binance.market_status_symbols, binance.symbols);
    ...
    async fn octobot_client_get_all_market_snapshots_rotates_exchange_symbol_subset_offsets() {
        ...
        Mock::given(method("GET"))
            ...
            .await;
        Mock::given(method("GET"))
            .and(path_regex(r"^/dashboard/watched_symbol/S03(%7C|\|)USDT$"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "exchange_id": "binance-id",
                "time_frame": "1h"
            })))
    ...
    #[tokio::test]
    async fn octobot_client_get_all_market_snapshots_cools_down_unavailable_symbols() {
        let server = MockServer::start().await;
        ...
        Mock::given(method("GET"))
            .and(path("/api/get_all_symbols/binance"))
            .respond_with(
                ResponseTemplate::new(200).set_body_json(json!(["BAD/USDT", "GOOD/USDT"])),
            )
            .expect(2)
            .mount(&server)
            .await;
        ...
        Mock::given(method("GET"))
            .and(path_regex(r"^/dashboard/watched_symbol/BAD(%7C|\|)USDT$"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "exchange_id": "binance-id",
                "time_frame": "1h"
            })))
            ...
            .await;
        Mock::given(method("GET"))
            .and(path_regex(r"^/dashboard/watched_symbol/GOOD(%7C|\|)USDT$"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "exchange_id": "binance-id",
                "time_frame": "1h"
            })))
        ...
        Mock::given(method("GET"))
            .and(path_regex(
                r"^/dashboard/currency_price_graph_update/binance-id/BAD(%7C|\|)USDT/1h/live$",
            ))
            .and(query_param("display_orders", "false"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "error": "No data"
        ...
        Mock::given(method("GET"))
            .and(path("/api/market/ticker"))
            .and(query_param("exchange", "binance"))
            .and(query_param("symbol", "BAD/USDT"))
            .respond_with(ResponseTemplate::new(404))
            .expect(1)
            .mount(&server)
            .await;
        ...
        Mock::given(method("GET"))
            .and(path_regex(
                r"^/dashboard/currency_price_graph_update/binance-id/GOOD(%7C|\|)USDT/1h/live$",
            ))
            .and(query_param("display_orders", "false"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "candles": {
        ...
        let client = OctobotClient::new(&server.uri(), None, 10.0);
        let first = client.get_all_market_snapshots(&[], &[], 1).await;
        assert_eq!(first.len(), 0);
        let second = client.get_all_market_snapshots(&[], &[], 1).await;
        assert_eq!(second.len(), 1);
        assert_eq!(second[0].symbol, "GOOD/USDT");
        server.verify().await;
    }

    #[tokio::test]
    async fn octobot_client_get_all_market_snapshots_prefers_market_status_symbols_over_exchange_universe()
     {
        let server = MockServer::start().await;
        ...
        let client = OctobotClient::new(&server.uri(), None, 10.0);
        let snapshots = client.get_all_market_snapshots(&[], &[], 100).await;
        assert_eq!(snapshots.len(), 2);
        assert!(
            snapshots
                .iter()
                .any(|snapshot| snapshot.symbol == "BTC/USDT")
        );
        assert!(
            snapshots
                .iter()
                .any(|snapshot| snapshot.symbol == "DOGE/USDT")
        );
        assert!(
            snapshots
                .iter()
                .all(|snapshot| snapshot.symbol == "BTC/USDT" || snapshot.symbol == "DOGE/USDT")
        );
        server.verify().await;
    }
